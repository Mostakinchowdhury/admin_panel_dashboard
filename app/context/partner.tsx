...
export const Partnerpageprovider = ({
  children,
}: {
  ...
  return (
    <partnerpagecontext.Provider value={values}>
      {children}
...
export default function usePartnercontext() {
  const context = useContext(partnerpagecontext);
  if (!context) {
    throw new Error(
      'usePartnercontext must be used within a Partnerpageprovider',
    );
  }
  return context;
}
