...
export const Riderpageprovider = ({
  children,
}: {
  ...
  return (
    <riderpagecontext.Provider value={values}>
      {children}
    </riderpagecontext.Provider>
  );
};

export default function useRidercontext() {
  const context = useContext(riderpagecontext);
  if (!context) {
    throw new Error('useRidercontext must be used within a Riderpageprovider');
  }
  return context;
}
